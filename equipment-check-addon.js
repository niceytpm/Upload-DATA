...
function unique(field){return [...new Set(state.rows.map(r=>normalizeText(r[field])).filter(Boolean))].sort((a,b)=>a.localeCompare(b,undefined,{numeric:true,sensitivity:'base'}))}
...
function setupSuggest(input,field){
  ...
  const render=()=>{
    const query=normalizeText(input.value);let values=unique(field);
...
async function openCheck(){
  if(!me||!canUpload(me.role))return;closeMenu();syncFactories();showOnly('machineCheckView');
...
function wire(){
  ...
  const oldRenderRole=window.renderRole||renderRole;window.renderRole=renderRole=function(){oldRenderRole();const btn=q('btnOpenMachineCheck');if(btn)btn.classList.toggle('hidden',!canUpload(me?.role))};
