...
async function openCheck(){
  if(!me||!canManage(me.role))return;closeMenu();syncFactories();showOnly('machineCheckView');
...
function wire(){
  ...
  const oldRenderRole=window.renderRole||renderRole;window.renderRole=renderRole=function(){oldRenderRole();const btn=q('btnOpenMachineCheck');if(btn)btn.classList.toggle('hidden',!canManage(me?.role))};
